...
def make_reset_fn(type: str | None, track: Track, num_agents: int, **kwargs) -> ResetFn:
    ...
    try:
        ...
        refline = {
            "cl": [track.centerline],
            "rl": [track.raceline],
            "shuf": [track.raceline, track.centerline],
        }[refline_token]
        reset_fn = {"grid": GridResetFn,
                    "random": AllTrackResetFn}[reset_token]
        shuffle = {"static": False, "random": True}[shuffle_token]
        options = {
            "cl": {"move_laterally": True},
            "rl": {"move_laterally": False},
            "shuf": {"move_laterally": False},
        }[refline_token]
